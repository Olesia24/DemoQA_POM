...
public class PracticeFormPage extends BasePage {
    ...
    public PracticeFormPage enterBirthday(String date) {
        clickWithJS(dateOfBirthInput, 0, 200);//focus on the field
        String os = System.getProperty("os.name");//select os
        System.out.println("My OS is "+ os);
        if (os.startsWith("Mac")){ //press on the keyboard 2 keys -CONTROL-Wind COMMAND-Mac
            dateOfBirthInput.sendKeys(Keys.COMMAND,"a");
        }else {
            dateOfBirthInput.sendKeys(Keys.CONTROL,"a");
        }
        dateOfBirthInput.sendKeys(date); //enter text
        dateOfBirthInput.sendKeys(Keys.ENTER); //press on the keyboard the key ENTER
        return this;
    }
    //enter subjects
